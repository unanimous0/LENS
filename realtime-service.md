...
- **비율은 부호 스케일 표기** — +1.24 = 매수잔량 1.24배 / −1.35 = 매도잔량 1.35배, 균형 1.00.
  내부 플롯은 0 중심 연속값 v (±1 사이 공백 점프 방지) — 정본 `components/futures/session.ts`의
  `ratioValue`/`fmtRatio`. MA·분포 통계도 v 스케일(방향 중립).
- 세션 = **08:45~15:45 KST** (지수선물 정규장, 주식보다 15분 이름). 세션 밖 표본/틱은 차트 제외.
  시간축은 고정이 아니라 **auto-fit**(개장~현재 데이터를 항상 전체 폭에) — 수동 줌/팬 시 자동 맞춤
  중지, 더블클릭으로 복귀. 세션 내 표본 0이면(장외 mock) fitContent 폴백.
- 분봉(1/3/5/10/15/30/60분) **클라이언트 집계** (10초 샘플 근사 — 샘플 사이 고저 미포착), 비율은
  라인/캔들 선택. 비율 MA 5/15/60분(시간창 평균, 창 미충족 구간 미표시 + 칩 대기 표시). 당일 비율
  통계(percentile·고저·평균·매수우위 비중)도 클라이언트 계산 — 서버/DB 무관. 표시 선호는 localStorage.
- 페이지 하단: 베이시스 미니 차트 2열 (시장 = p−u, 이론 = th−u). OI는 카드 숫자로만 (차트 제거).
- 상승/하락 색은 이 탭 한정 TV 팔레트 `#089981`/`#f23645` (`docs/화면 캡처 2026-08-12 094528.png` 기준).
...
| 시간대 | 키A (아버지) | 키B (와이프) |
|---|---|---|
| 09:00 ~ 15:45 KST | LENS WS 코어(현물·선물·주식) + 부족 시 REST | **LENS REST + ETF iNAV/호가 WS** (아래 "WS 두 키 분산") |
| 15:50 ~ 익일 08:50 | LENS WS + REST | **Finance_Data 일배치** (키B WS는 15:45 자동 해제) |

구현 (`realtime/src/feed/ls_rest.rs`):
- `KeyPool::from_env()` — `LS_APP_KEY_A`/`_B` 필수 (legacy `LS_APP_KEY` fallback 제거됨, PR-17)
- `is_lens_rest_window_now()` — 09:00~15:45 KST 판정
- `rest_credentials()` — 시간대 보고 (key, secret) 반환. 모든 REST 호출 site가 이 helper 호출
- token cache: `HashMap<app_key, CachedToken>` (키별 분리) — 키A WS 토큰과 키B WS/REST 토큰 독립
- 경계 5분 안전 마진 (FD는 15:50/08:50까지 작업 끝냄)

##### WS 두 키 분산 (2026-06-12) — 위 "진화 경로" 구현됨

증상: 외부망 ETF 차익 화면에서 **호가가 멈췄다 받아졌다** 반복 + 데이터 시작이 ~30초 지연.
원인: 키A 한 계정에 ~9연결/800+구독(현물 fixed + 선물 273 + ETF S3_/I5_/호가)이 몰려
**LS가 키A WS 세션을 강제 종료**(`TLS error: the SSL session has been shut down`) → idle 30s →
재연결 storm (backoff 2→4→…→32s = 그 30초). `/debug/stats.reconnect_count` 폭증으로 확인.

해결: ETF 화면 전용 무거운 스트림 **iNAV(I5_) / 호가(H1_)를 키B(와이프 계좌) WS로 분리**.
키A·키B는 별개 증권 계정이라 각자 독립 WS 한계 → 실질 용량 2배.

| 스트림 | 키 | 비고 |
|---|---|---|
| 현물 fixed / 선물 JC0 / 주식 S3_ | 키A (영구) | 종목차익·LP·대시보드 코어 |
| ETF iNAV(I5_) / 호가(H1_) | **키B** | ETF 차익 화면 전용, **09:00~15:45만** |

구현 (`ls_api.rs`):
- `ws_key_b_credentials()` (`ls_rest.rs`) — 키B 명시 반환, 미설정이면 None → 키A 폴백(`kb_active=false`)
- `spawn_inav_connections`/`spawn_orderbook_connection`에 `key_b: bool`. 키B면:
  - 연결 루프 윈도우 게이트: `!is_lens_rest_window_now()`이면 15초 대기 후 재확인(종료 X) →
    장 개시 전 구독이 09:00에 자동 시작. (최대 ~15초 지연)
  - **watchdog**(연결당 1개, 30s 폴링): 윈도우 open→closed 전이 시 connection cancel →
    15:45 키B WS 강제 해제 (15:50 FD 침범 방지). 실은 장 마감 후 I5_ 끊김→idle→윈도우 게이트가
    암묵적 2차 방어선이라 watchdog 죽어도 ~15:45 닫힘.
- 한계: 이번 분산은 iNAV/호가만. **subLimit을 50→100/200으로 올리면** 키A 주식(S3_) 연결이
  늘어 재포화 가능 → 그땐 주식도 분산 필요. 키B 장중 차단 시 런타임 키A 폴백 없음(재시작 필요,
  단 가격·거래대금은 키A REST로 유지되어 "계산중" degraded-graceful).

##### WS connect 타임아웃 (2026-06-12)

`run_single_connection`의 `connect_async_tls_with_config`에 명시적 타임아웃 없어, LS가 접속을
늦게 받으면 커널 기본 TCP 타임아웃(~30s)을 그대로 기다린 뒤 `os error 110`. → `tokio::time::timeout(10s)`로
감쌈. 느린 connect는 10초에 끊고 짧은 backoff로 재시도. 모든 연결 그룹 공유 경로라 일괄 적용.
...
- [x] **futures_master.json stale 감지 (PR-15c)**: `/debug/stats.futures_master.{loaded_mtime, current_mtime, stale}` 노출. 주식선물 만기(매월 *두 번째 목요일*) 다음날 새벽 Finance_Data가 파일 갱신 → realtime은 startup 1회 로드(hot-reload 미지원)라 *수동 재시작 필요*. stale=true이면 사용자가 인지해서 `start_dev.sh` 재실행.
...
| `usePageInavSubscriptions` | `/realtime/subscribe-inav`(`/unsubscribe-inav`) | `SubscribeInav` / `UnsubscribeInav` | **ref-count + warm-down** (I5_ ETF iNAV, stocks와 동일 패턴·env 공유) | ETF주선교체 |
...
**Tier 1 (`POST /permanent-stocks`)**: backend가 set 전체를 replace. realtime이 diff 계산 → 내부적으로 `SubscribeStocks`/`UnsubscribeStocks`로 변환 → T2 흐름 위에 자연스럽게 얹힘 (영구 코드는 client_id 없이 호출 → client_subs 미추적). backend는 포지션 등록/청산/삭제 시점에 `asyncio.create_task`로 fire-and-forget 호출 + startup 시 1회.

**Tier 1 견고성 — realtime startup polling**: realtime이 단독 재시작되면 영구 set 메모리 손실. backend가 push해주기 전까지 공백 발생. → realtime startup task가 `GET /api/positions/active-leg-codes` (전용 lightweight endpoint)를 5초 간격 최대 12회 retry. 첫 성공 시 종료 + 이후는 backend push에 위임. backend가 끝까지 안 떠있으면 give up — 다음 포지션 변경 시점에 push로 자동 회복.
...
**Tier 2가 적용되는 그룹** (warm-down + ref-count):
- `/subscribe-stocks` (S3_/K3_): 주식·ETF 누적
- `/subscribe-inav` (I5_): ETF iNAV 누적

두 그룹은 동일 `REALTIME_WARM_DOWN_SECS` 값 공유. 서로 별도 `pending_drops` / `drop_*_rx` 채널이라 간섭 없음.

**Tier 2가 다루지 않는 그룹** (replace 시맨틱이라 부적합):
- `/subscribe` (선물): stock-arbitrage 월물 토글. set 전체 교체.
- `/orderbook/subscribe-bulk`: 호가창 단일 active 셋.

**t1102 ETF 분기 (2026-05-16 추가)**: `fetch_stocks_initial`이 `etf_codes` set으로 ETF 여부 판단 → ETF면 `EtfTick`으로, 주식이면 `StockTick`으로 emit. 평일 LS WS attach 시 S3_/I5_ 스트림이 자동 분기하던 것을 *t1102 단독 시점(휴장·시작 직후)에도 동일하게* 적용. frontend가 `etfTicks ?? stockTicks` fallback 없이 `etfTicks[code]` 직접 lookup 가능. `MasterShared.etf_codes`는 startup 시 backend `/api/etfs/pdf-all` items key 추출 (~636 ETF).
- 시작 직후 첫 I5_ 받기 전 nav=0 잠깐 표시 (bridge `etf_state_map`이 prev nav backfill로 1초 이내 정상화)
- 휴장엔 nav=0 영원이지만 NAV는 휴장에 의미 없음 → 통계차익 페어 상세 등 *price만 쓰는* 페이지는 영향 없음
