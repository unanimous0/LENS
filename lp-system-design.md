...
- 시트 5장:
  ...
  | `OMS` | 38행 — 함수 전략 v1.5 조건변수 **A~D**(초록 4칸, D = σ결합 연속 스큐) + 참고(μ괴리·전일종가·장중상한)·비고 (이력은 바로 아래) |
...
#### OMS 조건변수 시트 — 함수 전략 v1.5 (2026-09-08)

엑셀 `OMS` 시트가 OMS 조건변수 A~D를 그대로 반입하는 표를 제공한다 (A·B·D는 % 부호 입력).
v1.5 = 분기·임계·정리 앵커 전부 폐지, 연속 스큐 일원화: 호가 = 진입 밴드 − D×포지션/C,
D = σ결합 (한도에서 백테스트 청산 레벨 μ±0.5σ, 절반에서 μ±1σ — 2σ 기본은 실운영 '절반도 안 찼는데 정리 세다'로 9/9 1σ 완화).
**전략·함수·변수 정의의 정본은 `docs/lp-oms-strategy-v1.html`** (md 중복 문서화 안 함 — 사용자 방침).
**화면 제안가에도 동일 편향 반영** (2026-09-09) — `/lp-desk` 제안매도/매수·x 컬럼이 밴드 − D×포지션/C까지 계산해 OMS가 실제로 거는 자리를 보여준다 (`lp-desk.ts` `inventoryBiasBp`/`inventoryCapShares`, 상수는 백엔드와 동일).
**E = 정리 강도(σ 배수)** — 정리 레벨은 엑셀이 아니라 OMS 함수가 μ=(A+B)/2와 D로 직접 계산한다.
구현: `lp_desk_export.py` `_build_oms` (A/B/D는 `quote_band_bp` 그대로, E는 상수 `UNWIND_SIGMA`=1.0,
비고 검사 A>B · 0≤E<z). 이력: 8/30 a~e → 9/7 v1.1 A~D → 9/8 v1.2 E·F 정리 앵커 → 9/8 v1.3 E 정리 강도
→ 9/8 v1.4 F=μ 직접+임계 G → 9/8 v1.5 전부 폐지·연속 스큐 D=2σ 일원화(사용자: 임계 대신 수량 비례 가중).
