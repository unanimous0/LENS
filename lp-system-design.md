...
**양방향 조율 프로토콜** (콘텐츠가 아닌 *규칙*만 영구화 — 시점 스냅샷은 §13에 두고 머지되면 폐기):
- worktree1 → main: §12 객체 의존 목록 + 우리가 수정한 공유 파일을 핸드오프 프롬프트로 *능동 통지*
- main → worktree1: §12 객체의 스키마/적재/snapshot 변경, LS feed·구독·phase 변경, 공유 파일 수정분을 *회신* → worktree1은 이를 §13에 시점 액션으로 흡수
- 머지 완료 시 §13은 stale → 폐기. §12(규칙)만 영속
- 핵심 원칙: 데이터/인프라 변화는 git diff가 아니라 *문서 회신 + 머지 후 실행 검증*으로 푼다

---

## 13. 머지 후 필수 액션 (main 2026-05-17 회신 흡수 — 머지 완료 후 폐기)

> §12 프로토콜대로 시점 스냅샷. 머지+아래 액션 완료 시 이 섹션 삭제. ★ = 평일 재검증 전 필수.

### ★★ A1. 수정주가 전환 — `risk_estimator.py` 실제 결함 (CRITICAL)
main ⓐ: `ohlcv_daily.close_price`는 액면분할/병합 시 spike → OLS·잔차 공분산 무력화. **`adj_close` 필수.** `ohlcv_intraday` 직접 조회 금지 → view `ohlcv_intraday_adjusted`.
- `services/risk_estimator.py` `_fetch_market_returns`/`_fetch_stock_returns`가 `close_price` 직접 사용 → **`adj_close`로 교체.** 미전환 시 베타·잔차 전부 신뢰 불가. **§9.8에서 검증한 수치(베타 0.764/1.167 등)도 raw 기준이라 무효 — 머지 후 재산출.**
- `calc/pdf_basket.rs`/`stock_futures_intersect.rs`: basket NAV도 PDF 종목 수정주가 일관성 검토 (실시간 틱은 raw — 정합성 판단 필요)
- 영향 종목: `SELECT * FROM corporate_actions WHERE event_date > '2026-01-01'`. Finance_Data 매일 04:30 갱신. 선물·지수는 분할 없어 raw OK. 출처: 머지본 CLAUDE.md + stat-arb-engine.md §12.8 (커밋 5d9263a)

### ★ A2. `realtime/src/main.rs` 머지 — 최고 신중 (merge --no-ff 합의)
main ⓑ: AppState에 `next_client_id`/`client_subs`/`permanent_codes` + subscription manager(ref-count, warm-down 60s, env `REALTIME_WARM_DOWN_SECS`) + client_id/hello 도입. 우리는 같은 파일에 `spawn_workers`(LP scheduler)+bridge hook 추가. **둘 다 보존 + init 순서 조율.**
- `useWebSocket.ts`: 우리 `fair_value_matrix`/`book_risk` dispatch + main hello/clientId **둘 다 살림**
- `marketStore.ts`(ⓘ): main이 `clientId`/`setClientId` 추가. `useLpInit` fetch에 `X-LENS-Client-Id` 헤더 패턴 적용 검토 (LP fetch는 1회성이라 server-side 구독 cleanup과 무관할 수 있음 — 평일 재검증 때 판단)
- `ls_rest.rs`(main): t1102가 ETF면 EtfTick·아니면 StockTick 분기 emit → 프론트 `etfTicks ?? stockTicks` fallback 제거 (우리 코드에 해당 fallback 있으면 정리)

### A3. 충돌 7파일 (main 확인필)
CLAUDE.md / backend/main.py / App.tsx / top-nav.tsx / useWebSocket.ts / realtime/src/main.rs / start_dev.sh — 대부분 추가형, 둘 다 살림. main.rs만 A2. **etfs.py(e85ca2b)·architecture.md·model/{message,mod}.rs는 main 미수정 → 충돌 없음 확인됨.**
- start_dev.sh: main이 stat-arb-engine(port 8300 별도 Rust binary) 빌드/실행 라인 + 시간가드 추가. 우리 backend 대기 가드와 **둘 다 살림** + 포트 8300 공존 확인

### A4. §9.6 해소 — 주식선물 5/14 롤오버 (main fc8dcc9 처리됨)
main이 `data/futures_master.json` 재생성 + `futures_master.py`(+402). `pdf_futures_match.py`는 파일 read라 머지 후 교집합 자동 반영 — 머지 후 1회 교집합 재산출 검증. **§9.6의 "수동 갱신 후순위"는 해소됨.**

### A5. §9.7 carry 정밀화 입력 — 대여요율 인프라 (main ⓖ)
main이 `services/loan_rates.py`(신규)+`GET /api/loan-rates/{code}`+SQLite+페어테이블 컬럼(4322319). 다음 빌드 carry 모델에서 "차입금리=대여요율" 일관 입력으로 **재활용**(별도 페이지 X). 인포맥스 theoretical_basis 내부 금리는 여전히 미상(ⓒ — §9.7 그대로).

### A6. 머지 후 필독 (새 단일 진실원)
- `realtime-service.md`(main +103): Tier 1/2/3 구독 정책 + 페이지별 라이프사이클 + t1102 ETF분기 + warm-down/startup polling. **LP가 실시간 WS 가격 입력 쓰면 이게 단일 진실원** — 우리 `calc/scheduler.rs` 가격 입력 경로를 이 정책에 맞춰 재검토
- `stat-arb-engine.md`(신규 +544): 별도 stat-arb 엔진(port 8300) 존재 인지. LP와 직접 무관하나 start_dev.sh·포트 공존

### A7. Finance_Data 후속 (main 모름 — 우리가 직접)
`futures_underlyings.stock_code` 34건 / `index_ohlcv_intraday` 코드통일 view — main 가시영역 밖. Finance_Data 측 직접 확인 (다음 빌드 진입 시).

### A8. 평일 재검증 시간 (ⓙ)
22:30~24:30 KST `start_dev.sh` 금지 (Finance_Data LS 일배치 공유). §9.8 재검증은 그 외 장중.
