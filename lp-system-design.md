...
- 여러 날 보유(베이시스 운용)는 v1 구조 그대로 컬럼 추가로 확장 가능하게만 설계 (보유일수·괴리 z).
  → **2026-08-28 백테스트로 기각** (괴리 회귀 반감기 0.28일 = 첫 밤에 종결). 남은 문제는 "며칠"이 아니라
  **"얼마까지 밤을 넘기나"** — 종목별 오버나이트 상한 **§14.12**.
...
- `GET /master` → `{stats_date, params:{window, **on_tail_loss_won,on_tail_z,on_max_resid_vol_bp,on_min_obs,on_cap_unit_won**}, calib_params:{...,g_window,s_window,**s_diff_horizons**:[60,120,300]}, items:[{etf_code,name,beta_k200,beta_kq150,r2,resid_vol_bp,resid_z,obs,**overnight_cap_won,overnight_banned**,creation_unit, prev_close,prev_close_date,prev_close_stale, gap_mean_bp,gap_sigma_bp,gap_obs, calib:{**g_mean_bp**, **g_sigma_level_bp**, **s_diff_sigma_bp**:{"60","120","300"}, s_inc_sigma_bp, **g_day_max**[], **g_day_min**[], g_quantiles:{p05..p95}, g_touch_days, g_days, g_bars, s_quantiles, touch_days, g_sigma_bp, days, bars} }]}` (gap_*·calib null 가능)
...
- **`GET /api/lp-desk/export.xlsx?z=&horizon=&intraday_cap_won=`** (`routers/lp_desk.py` → `services/lp_desk_export.py` 신규).
...
- 시트 5장:
  | 시트 | 내용 |
  |---|---|
  | `베타` | 36행 스냅샷 — 코드·종목명·β_K200·β_KQ150·R²·**잔차σ·O/N 상한(원)·O/N 금지**·μ괴리·σ괴리·σ선물(지평)·x매도·x매수·전일종가·CU. `/master` 캐시 그대로 (O/N 2열은 2026-08-28 추가, §14.12) |
  | `OMS` | 38행 — 함수 전략 v1.1 조건변수 **A·B·C·D**(% 단위, 초록 4칸) + 참고(μ괴리(%)·전일종가·장중상한)·비고 (2026-09-07 개편 — 구 a~e 폐기, 바로 아래) |
...
#### OMS 조건변수 시트 (2026-08-30) — 아침 반입용 a~e ⚠️ **2026-09-07 A~D로 대체됨** (바로 아래)

같은 파일 안에서 **한 걸음 더** 나간다. 위 `베타` 시트는 통계 스냅샷이라 사람이 읽고 해석해야
하는데, 실제 OMS에 넣는 건 종목별 숫자 5개뿐이다. 그 5개를 **그대로 옮겨 적는 표**로 분리했다.
사용자 OMS의 룰은 `재고수량 < c → (a 매도bp, b 매수bp)로 호가` / `재고수량 ≥ c → 정리 모드로
(d 정리매도bp, e 정리매수bp)로 전환`이고, 매일 아침 이 값들을 반입한다.

- **컬럼 12개**: 코드(텍스트 `@`)·종목명 / **a·b·c·d·e**(초록 = OMS 입력칸) / 참고 μ괴리·σ결합·
  전일종가·장중상한(원) / 비고. 참고 4열은 *a~e를 만든 입력*이라 표만 보고 손검산이 된다.
- **a·b = μ_g ± z·σ_comb** — `베타` 시트의 x매도/x매수와 **같은 값**이다. 두 시트가 각자 μ·σ를
  다시 조합하면 반올림 자리 하나로 파일 안에서 값이 어긋나므로, `quote_band_bp`가 `QuoteBand`
  NamedTuple(ask/bid/mu/σ_r/σ_comb)을 돌려주고 두 시트가 그 결과를 나눠 쓴다. z는 export의 기존
  `?z=` 그대로(기본 1.5) — 화면 튜너를 바꿔 받으면 a·b만 벌어지고 d·e는 μ에 남는다(실측 확인).
- **d = e = μ_g** (소수 1자리). 정리 국면에서 평상시 폭(±zσ)을 그대로 두면 나가야 할 쪽이 안 붙는다.
- **c = 장중 재고 상한 ÷ 전일종가, 100주 내림**. 상한은 `?intraday_cap_won=`(기본 **3억**,
  2026-08-27 실전 분석의 최적 2.5~3억 구간). **§14.12 O/N 상한과는 다른 축**이다 — O/N은 잔차 σ의
  함수(종목별, 밤을 넘기는 리스크)이고, 이건 낮 동안의 정리 여력 제약이라 전 종목 동일 금액이다.
  내림인 이유는 c가 *정리 모드 전환 문턱*이라 반올림으로 상한을 넘으면 안 되기 때문. O/N 금지
  종목도 장중 상한은 같은 로직이고, 비고에 **"당일 정리 필수"**만 붙는다(값은 그대로 쓴다).
- **비고 = 그대로 반입하면 안 되는 이유만** (정상 행은 빈칸): `통계 없음`(캘리브 없음 → a~e 빈칸,
  c는 전일종가만 있으면 나온다) · `전일종가 없음`(c 빈칸) · `전일종가 지연` · `당일 정리 필수`.
- **PARAMS**에 메타 `장중 재고 상한 (원)` 1줄 + 사용법 `■ [OMS] 조건변수 a~e` 블록. 마지막이
  ⚠️ **정리 모드에서 초과 방향의 반대쪽 호가는 μ−2.5σ 이하로 깊게 두거나 수량 0** — d·e를 둘 다 μ에
  두면 양쪽이 같은 확률로 체결돼 **재고가 제자리다**(롱 초과면 매수 e를 μ−2.5σ 이하로, 숏 초과면
  매도 d를 μ+2.5σ 이상으로). 정리 모드는 "좁게"가 아니라 **한쪽만** 세우는 것이라는 게 요점.
- 승수 셀 주소를 라벨에서 역산하는 기존 설계(§14.11) 덕에 PARAMS 메타가 한 줄 늘어도 `헤지` 수식은
  자동으로 따라갔다(`PARAMS!$B$9→$B$10` 실측 확인) — 상수로 박았으면 조용히 깨졌을 자리다.
- **검증 (2026-08-30, 실서버 xlsx)**: 38행 · a>d=e>b 전행 성립 · a·b가 `베타` x와 전수 일치 ·
  손검산 3종(396500 8,500주×34,900=296.65백만 / 0117V0 15,000주×19,910=298.65백만 /
  472150 14,900주×20,080=299.19백만 — 모두 3억 아래 최대 100주 배수, +100주면 초과) ·
  `intraday_cap_won=2억` → c만 비례 축소(8,500→5,700)하고 a~e 불변 · 코드 텍스트 서식 보존 ·
  O/N 금지 12종 비고 표시 · calib None/μ 결측/전일종가 None 3종 합성 케이스 정상 degrade.
  기존 4시트는 값·수식 회귀 없음(`체결` 전수 동일, `베타` x는 종전 산식으로 독립 재계산 일치).

#### OMS 시트 A~D 전면 개편 (2026-09-07) — 함수 전략 v1.1 조건변수

OMS 호가를 **함수 3개(70 LP_재고편향 · 71 LP_매도호가 · 72 LP_매수호가)**로 옮기면서 시트가 넘기는
조건변수 자체가 바뀌었다. 전략 정본은 `docs/lp-oms-strategy-v1.html` (v1.1):
...
- **구 a~e 폐기**. `재고 ≥ c`에서 정리 호가(d·e)로 점프하던 스위칭은 **재고에 비례한 연속 편향**이
  같은 역할을 한다 — 재고가 C에 닿으면 매도호가가 정확히 정리 레벨(μ+0.5σ)에 오고 초과분은 더
  밀린다. 절벽·깜빡임이 구조적으로 없어 d·e 컬럼과 "반대쪽을 μ−2.5σ로 비켜라" 운영 각주가 통째로
  사라졌다.
- **컬럼 10개**: 코드(텍스트 `@`)·종목명 / **A 매도(%)·B 매수(%)·C 재고한도(주)·D 스큐σ(%·지평)**
  (초록 4칸) / 참고 μ괴리(%)·전일종가·장중상한(원) / 비고.
- **단위가 bp → %**. OMS 조건변수는 % 부호 입력이라 `A·B·D = bp ÷ 100`, **소수 3자리**(변수 입력
  5자리 제한 — 전략 체크리스트 #7). 산식은 여전히 `quote_band_bp` 한 곳뿐이고 `_pct`가 단위만
  옮긴다 — `베타` 시트 x(bp)와 OMS A·B(%)가 파일 안에서 어긋날 여지를 안 만든다.
  참고 μ도 %로 실어 **A ≈ μ% + z·D** 손검산이 표 안에서 닫힌다. σ결합 컬럼은 D와 중복이라 삭제.
- **D = σ_comb ÷ 100**. 스큐 강도는 밴드를 만든 σ와 **같은 σ**라야 "한도에서 정리 레벨에 닿는다"는
  설계가 성립한다 (z를 바꿔 받으면 A·B만 벌어지고 D는 그 z의 1σ 그대로).
- **구 E 재고중심·F 틱여유는 전략 v1.1(2026-09-07 개정)에서 폐지** — 편향 원점이 0으로 붙박이라
  `재고 − E`가 `포지션`이 됐고, 함수 안 자전·침범 가드도 없앴다(시장을 관통한 체결가는 상대방 호가라
  손해가 아니고, 시차 자전은 **자전방지 플래그**가 전담). 상수 2개(`OMS_INVENTORY_CENTER`/
  `OMS_TICK_WON`)와 컬럼 2개가 함께 사라져 초록칸이 6 → 4로 줄었다. **A~D 값·산식은 불변**.
- **A > B 검증**(간격 = A−B > 0 이라야 자기 크로스가 산술적으로 불가). 산식상 A−B = 2z·σ_comb라
  정상 경로에선 안 깨지지만 σ→0에서 %로 반올림하며 붙는 경계가 있다. 예외를 던져 파일 전체를
  못 만들게 하는 대신 **그 행 비고에 `⚠ A≤B 반입 금지`** — 나머지 37행은 멀쩡하기 때문이다.
- **PARAMS 사용법** `■ [OMS] 조건변수 A~D` 블록 교체: 함수 3개 산식·% 부호 규약·3자리·C 반값
  운영(험한 날)·구 a~e 및 E·F 폐기 명시 + ⚠️ **전 주문 자전방지 플래그 활성화**(시차 자전 전담)
  리마인더.
- **검증 (2026-09-07, 실서버 xlsx + openpyxl)**: 38행 전수 — A·B·C·D·μ를 `/master` calib에서 독립
  재계산해 셀 단위 일치, A>B 전행 성립, `베타` x(bp) ÷100 = A·B 전수 일치, 코드
  텍스트 서식·초록 4칸/참고열 무채색 확인. 손검산 3종 —
  396500(μ −12.07 · σ_g 5.24 · σ_r 4.47 → σ_comb 6.89bp) **A −0.017 · B −0.224 · D 0.069 · μ −0.121 ·
  C 8,500**(3억÷34,940) / 417630 **0.224 · −0.446 · 0.223** / 472150 **0.102 · −0.190 · 0.097 · C 15,100**.
  축 분리도 확인: `intraday_cap_won=2억` → C만 8,500→5,700(A·B·D·μ 불변), `z=2.5&horizon=300` →
  A·B·D만 확대(396500 0.116 / −0.357 / 0.094, C·μ 불변, D 헤더 `(%·5분)`). 엣지 5종(calib None·
  μ 결측·전일종가 None·σ=0·지연+O/N금지) 비고 정상. O/N 금지 12종 `당일 정리 필수` 유지.
  **기존 시트 회귀 없음** — 손댄 곳은 `_build_oms`/`oms_note`/PARAMS 사용법 텍스트뿐이고, §14.11
  초판(HEAD) 빌드와 셀 단위로 비교하면 `체결`은 전수 동일, 나머지 차이는 전부 *이번 이전의 미커밋
  기능*으로 설명된다: `베타` 353건 = O/N 2열 삽입에 따른 열 이동(헤더 10 + 38행×9), PARAMS 메타
  6건 = `장중 재고 상한` 1줄 추가, `헤지` 2건 = 그 때문에 승수 셀 주소가 `$B$9→$B$10`로 자동 추종
  (라벨 역산 설계가 의도대로 동작).
  **E·F 축소(v1.1) 재검증** — 같은 파라미터의 A~F 빌드와 셀 단위 대조: OMS 잔여 10열 값 전수 동일
  (컬럼만 12→10), `베타`·`체결`·`헤지` 전수 동일, PARAMS는 생성시각 외 차이가 사용법 9줄 교체뿐
  (`intraday_cap_won=2억`·`z=2.5&horizon=300` 두 조합에서 각각). 서식은 A~D 4칸만 초록.
...
### 14.12 오버나이트 상한 (2026-08-28 백테스트 확정)

**질문을 바꿨다.** 종전 v1 전제는 "여러 날 보유(베이시스 운용)도 된다"였는데, 백테스트가 그
전제를 기각했다. 남는 질문은 **며칠이 아니라 얼마까지**다 — 종목별로 밤을 넘길 수 있는 **평가액
상한**을 두고, 그 위는 14:30 전에 정리한다.

**백테스트 결론 (근거, 2026-08-28):**
- 괴리 회귀는 **첫 밤에 종결**된다 — 반감기 **0.28일**. 하루를 더 들고 있어야 할 이유가 없다
  (N ≥ 2일 보유의 근거는 어느 레짐에서도 나오지 않았다).
- 기대값 대비 위험이 압도적이다 — 1박 기대 수익 **≈ 30bp** vs 잔차 σ1d **중앙 211bp**. 평균으로
  이기는 게임이 아니라 **꼬리로 지는 게임**이다.
- 두 레짐(고변동/저변동) 모두에서 살아남은 건 **잔차 σ가 작은 지수형**뿐. 섹터·테마형은 한 레짐에서
  이겨도 다른 레짐에서 그만큼 토해냈다.

**룰 (확정):**
```
overnight_cap_won = 500,000 / (1.645 × resid_vol_bp × 1e-4)     # ≈ 30.4억 ÷ resid_vol_bp
단, resid_vol_bp > 250      → cap 0 (금지)
    회귀 obs < 60 (신규상장) → cap 0 (금지)
    회귀 표본 부족(σ 없음)   → cap null (금지 아님 — "모름")
```
- 의미: **1박 5% 꼬리손실(1.645σ)이 50만원을 넘지 않는 평가액**. 새 통계는 없다 — §14.3 2-팩터
  회귀의 잔차 σ(bp/일)를 그대로 쓴다.
- **"금지"와 "모름"은 다른 상태**다. 금지는 σ를 재 봤더니 크더라는 판정(cap 0), 모름은 아직 못
  쟀다는 뜻(cap null)이라 화면·엑셀 문구가 다르다. 신규상장은 σ가 있어도 표본이 얕아 못 믿으므로
  금지 쪽에 둔다.
- 표시는 **백만 단위 내림** (보수적 방향, 주문은 어차피 단위로 끊는다).
- 롱·숏 구분 없이 **|평가액|**으로 비교한다 — 1박 잔차 리스크는 방향과 무관(§14.4 잔차 합산과 같은 규약).

**구현 (2026-08-28):**
- **백엔드** `services/lp_desk_stats.py` — 상수 `ON_TAIL_LOSS_WON`(50만) / `ON_TAIL_Z`(1.645) /
  `ON_MAX_RESID_VOL_BP`(250) / `ON_MIN_OBS`(60) / `ON_CAP_UNIT_WON`(100만) + `_overnight_cap()`.
  `/master` items에 `overnight_cap_won`·`overnight_banned`, params에 룰 상수 5개를 실어 보낸다 —
  **숫자의 단일 진실원은 서버**이고 화면·엑셀은 문장에 끼우기만 한다. 상한은 반올림된 표시용
  `resid_vol_bp`로 계산한다(툴팁 산식을 손으로 따라가도 같은 값이 나오게).
- **프론트** `pages/lp-desk.tsx` — 포지션 그룹 `수량` 오른쪽에 **O/N 상한** 컬럼(정렬 가능).
  포지션이 없어도 상한은 보인다(체결 *전에* "여긴 얼마까지"를 알아야 한다). 금지·표본부족은
  `—`(text-t4) + 사유 툴팁. **|평가액| > 상한이면 셀 경고 톤 + 둘째 줄에 초과분** = 14:30 전에 줄일 금액.
- **엑셀** `services/lp_desk_export.py` — `베타` 시트 잔차σ 옆에 `O/N 상한(원)`·`O/N 금지`
  (`금지`/`표본부족`/빈칸). 조회 키 열(A~D)은 그대로라 `체결` 시트 VLOOKUP은 무영향. PARAMS
  사용법에 산식·금지 조건 2줄(숫자는 `params`에서 받아 쓰고, 없으면 그 2줄을 아예 뺀다).
- **실측 분포 (2026-08-28, stats_date 08-27, 38종)**: 허용 **26종** / 금지 **12종** / 표본부족 0.
  최대 472150 σ49.7bp → **61백만**, 292150 σ64.0 → 47백만, 138520 σ120.7 → 25백만, 396500 σ161.5 →
  18백만, 하한권 138540 σ248.1 → 12백만. 금지 12종은 471760(255.3)·305540(267.6)·157500(268.5)·
  462010(269.1)·494670(284.6)·139220(285.9)·377990(292.0)·364980(296.2)·0091P0(297.3)·0117V0(328.6)·
  138530(335.3)·0148J0(362.0) — **2차전지·조선·원자력·AI전력기기 등 테마형이 그대로 걸린다**.
- ⚠️ 상한은 σ가 갱신되면(매일 회귀 재계산) 같이 움직인다. 어제 허용이던 종목이 오늘 금지가 될 수
  있으므로 **판단은 장중 화면·당일 엑셀 기준**이다. 잔차 σ 자체의 한계(창 120일 · 레짐 전환에
  후행)는 그대로 물려받는다.
