...
**112종 (2026-09-15 확장)** — 이력: 36종(8/20 사용자 확정) → 38종(472150·417630) → 사용자
"etf 리스트" 엑셀(`docs/etf 리스트.xlsx`, 112종) 기준 74종 추가. 브랜드 분포 TIGER 80 / SOL 16 /
RISE 14 / HANARO 1 / PLUS 1. 매핑은 FD `etf_master_daily.kr_name` 정규화 매칭(공백 제거) —
TIGER 코스닥글로벌 1종만 FD 마스터 부재(상장폐지 추정)로 제외.
정의 파일: `backend/data/lp_desk_universe.json` (코드 배열 — 수정만으로 유니버스 변경, mtime+md5
핫리로드). **커밋 필수** (누락 시 /master 503).

- 캘리브 결측 3종 (PDF 재구성 한계): `0162Z0`(채권혼합) `0223A0`(신규 커버드콜, β도 없음)
  `329200`(리츠) — 화면 "—"+사유, 엑셀 OMS A·B 빈칸이라 호가 조건변수 미반입. C(한도)는 전 종목 산출.
- calib refresh 38종 13초 → **112종 약 150초** (1시간 캐시 — 아침 첫 갱신만 체감).
- **운용사 브랜드 칩 필터** (2026-09-15, lp-desk.tsx): 종목명 첫 토큰에서 동적 도출(KODEX·ACE
  추가 시 자동), 단일 선택 + localStorage. **표만 필터** — 집계(버킷·미헤지 델타·잔차·P&L)·실시간
  구독은 전체 유지, 필터 밖 브랜드에 보유가 있으면 "숨은 보유 N종" 배지. 112행 렌더 실측 60fps
  (가상화 불필요, 전면 재정렬 89~122ms).
- ⚠️ 키B WS(iNAV I5_+호가 H1_) 구독이 38→112종 3배 — 장중 호가 스톨 재발 시 1순위 용의자
  (구독을 필터 연동으로 줄이는 옵션 보류).
...
- 선물 30초봉: 초기엔 LS 500봉/일 상한으로 11:26~만 가정했으나 **FD 실측(2026-09-16) 풀세션(08:45~15:45) 적재 확인** (2026-01-02부터 전 기간) — s 표본도 g와 같은 클린창 09:10~15:20 전체다. 쿼리가 시간창 파라미터라 코드 수정 없이 자동 확장돼 있었다.
...
5. **캘리브 표본 창 표시** — `calib_params.s_window`에 **실측** 시각 범위를 담아 헤더에 노출 (실측이라 풀세션 적재 후 자동으로 `09:10~15:20` 표시 — 2026-09-16 확인).
...
- 시트 5장:
  ...
  | `OMS` | 38행 — 함수 전략 v1.6 조건변수 **A~G**(초록 8칸: A·B 진입 밴드 / C 재고한도 / D 1×σ결합(앵커 눈금) / E 정리 D배수 / F μ괴리 / G 정리 시작 재고 / H 0.5×σ결합(편향 눈금)) + 참고(전일종가·장중상한)·비고 (이력은 바로 아래) |
...
#### OMS 조건변수 시트 — 함수 전략 v1.6 (2026-09-10)

엑셀 `OMS` 시트가 OMS 조건변수 A~G를 그대로 반입하는 표를 제공한다 (A·B·D·F·H는 % 부호 입력).
v1.6 = **진입 밴드 + 재고 방향만 고정 정리 앵커 + 양쪽 공통 연속 편향**: 편향 = H×포지션/C,
매도앵커 = 포지션 > G면 F+E×D 아니면 A, 매수앵커 = 포지션 < −G면 F−E×D 아니면 B, 호가 = 앵커 − 편향.
반대쪽은 밴드에 남아 편향만큼 후퇴할 뿐이라 정리 중에도 싸지면 계속 매집한다(v1.4와의 차이).
**전략·함수·변수 정의의 정본은 `docs/lp-oms-strategy-v1.html`** (md 중복 문서화 안 함 — 사용자 방침).
**화면 제안가도 같은 산식** (2026-09-09 편향 / 2026-09-10 앵커) — `/lp-desk` 제안매도/매수·x 컬럼·체결
임박 판정이 전부 "OMS가 실제로 거는 자리"를 본다 (`lp-desk.ts` `suggestQuote`, 상수 `BIAS_SIGMA_MULT`·
`UNWIND_SIGMA`·`UNWIND_MIN_SHARES`는 백엔드와 동명·동값). 정리 모드 행은 호가 셀 툴팁·g 분포
마커 라벨에 "정리 앵커 μ±1σ" 근거가 붙는다.
구현: `lp_desk_export.py` `_build_oms` (A/B/D/F/H는 `quote_band_bp` 그대로, E·G는 상수 `UNWIND_SIGMA`=1.0 ·
`UNWIND_MIN_SHARES`=1,000 전 종목 동일, 비고 검사 A>B · 0≤E×D<z·σ · B<F<A). 이력: 8/30 a~e → 9/7 v1.1 A~D
→ 9/8 v1.2 E·F 정리 앵커 → 9/8 v1.3 E 정리 강도 → 9/8 v1.4 F=μ+임계 G → 9/8 v1.5 연속 스큐 일원화
→ 9/10 v1.6 정리 앵커 복원(재고 방향만) + 소량 보호 G.
**9/9 분리형 전환 (A~G 8변수)** — 시작은 "편향이 너무 세다"(사용자)였고, 1차 조정은 D 하나로 편향과
앵커 눈금을 겸한 채 D 0.5σ·E 2.0으로 상쇄시켰다. 두 손잡이가 묶여 있어 한쪽만 못 만지므로 **눈금을
분리**했다: **D = 1×σ결합(정리 앵커 눈금 전용, `ANCHOR_SIGMA_MULT`)** · **E = 1.0(D 배수)** → 앵커 μ±1σ ·
**편향 = D÷2 함수 내장(재고 편향 전용, `BIAS_SIGMA_MULT`)** → OMS 함수 72가 D 대신 **H**를 읽는다(함수 코드
변경은 이 한 곳). **물러남은 H만, 정리 레벨은 E·D만.** 수치 동작·화면 제안가는 1차 조정과 완전 동일
(편향 한도에서 0.5σ = 10%당 0.05σ, 앵커 1σ). D=1σ라 앵커 제약은 `0≤E<z`로 읽히고(코드는 곱
`UNWIND_ANCHOR_SIGMA`=E×D÷σ 유지), 마감 스텝은 14:30 E 1.0→0.5 · 14:45 E→0 (H는 안 만짐).
미확인 1건: OMS 함수 71의 `0 - [조건변수.G]` 산술 지원 여부 (안 되면 음수 임계 변수 **I** 추가 — H는 편향).
