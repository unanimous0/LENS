...
- **상세 차트** (`components/flow/flow-detail.tsx`, 행 클릭) — alphasquare "투자자별 매매동향" 참고, 세로 4분할: ① 주가(수정종가 캔들 + 이평 50·100·200 + VWMA200 + 외인 평단 추정선) ② 외인 순매수 모멘텀(막대 + 5·20일선) ③ 누적순매수(외/기/개, 차트↔테이블) ④ 순매수 일별(외/기/개, 차트↔테이블). 1년/3년 토글, **4개 차트 시간축 동기화**(logical range subscribe — 세로 1열 필수, 2열은 우측 열 sync 깨짐). Lightweight Charts.
