...
- **상세 차트** (`components/flow/flow-detail.tsx`, 행 클릭) — alphasquare "투자자별 매매동향" 참고, 세로 4분할: ① 주가(수정종가 캔들 + 이평 50·100·200 + VWMA200 + 외인 평단 추정선) ② 순매수 모멘텀(외인/기관 토글, 막대 + 5·20일선 = 최근 매수 강도) ③ 누적순매수(외인/기관 토글, 누적선 + 20·60일 이평 + ▲골든/▼데드크로스 마커 = OBV+시그널선, 차트↔테이블) ④ 순매수 일별(외/기/개, 차트↔테이블). **이평 배치 원칙**: 추세·크로스는 추세를 갖는 누적(③)에서만 의미 → 일별(②)엔 강도용 단기 MA만. 1년/3년 토글, **4개 차트 시간축 동기화**(logical range subscribe — autoSize:true 필수, grid-cols-2에서 우측 열 생성 폭 0이면 sync 깨짐). Lightweight Charts.
