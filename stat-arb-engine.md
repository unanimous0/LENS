...
| 선물 종가·실측 베이시스 | `futures_ohlcv_daily` (`close`,`underlying_basis`) | `DISTINCT ON (underlying_code, contract_class)`로 종목×월물 최신 1행 |
...
- **롤**: front 잔존일 `< 2`면 back 월물. 만기일 당일 front를 잡으면 `d=0`이라 캐리가 정의되지
  않는다. 선택한 월물이 front면 `contract_class='NEAR'`, back이면 `'NEXT'` — **여기를 틀리면
  차월물 가격에 근월 베이시스를 붙이게 된다.** (2026-08-13 = 8월물 만기일이라 273종 전부 back.)
- **롤 경계 가드**: NEAR/NEXT 라벨만 믿지 않고 일봉의 `contract_code`가 마스터가 고른 월물
  코드(`ABK69000` 등)와 **같을 때만** 쓴다. 만기 다음 영업일 장중에는 마스터 front가 이미 신규
  월물인데 DB 최신 NEAR는 만기 소멸한 구월물(basis≈0)이라 라벨 조인이 엉뚱한 계약을 붙인다.
  불일치 종목은 items에서 제외 — 틀린 숫자보다 빈 값이 낫다. 제외 수는 응답
  `skipped_roll_mismatch`(평시 0, 프론트 미표시 — 진단용).
- **제외 조건**: `futures_underlyings` 매핑 없음 / 최근 5거래일 내 일봉 없음 / `spot ≤ 0`.
...
- 프론트 공용 타입·헬퍼: `frontend/src/lib/stat-arb/futures-carry.ts`
  (`loadFuturesCarry` / `buyLegKeys` / `carryOf` / `buyLegCarry` / `fmtValue`).
- 목록(`pages/stat-arb.tsx`): 정렬 가능한 **캐리(bp/일)** 컬럼 1개. 값 = 매수 종목의
  ...
  만기 이후 확정 배당락이 남은 종목만 값 옆에 **`배` 마커 1개**(10px, warning) — 툴팁에
  `만기(9/10) 이후 확정 배당락: 9/29 500원 — 롤 시 반영 필요`. 마커는 하나뿐이라 컬럼 폭·행
  높이는 그대로다.
- 상세(`pages/stat-arb-detail.tsx`): 하단 **"주식선물 대체"** 패널. 두 종목 각각 월물·만기·잔존일,
  실측/이론 베이시스(배당 반영 표시), 캐리(bp/일 · 만기까지 bp · 원/주), **계약 라운딩**
  (1계약 = multiplier주, 예시 `R 100주 : L β×100주` → 계약수 + 잔차 주수), 유동성, `data_date`.
  매수 종목을 강조하고 미상장이면 "주식선물 미상장 — 현물로만 실행". β<0이면 패널 상단에
  "두 종목 모두 매도(매수 종목 없음)" / "두 종목 모두 매수" 안내를 띄운다.
- 상세 카드 하단 **배당 3분류**(`DividendSection`): ① 만기 내 확정 — "이론가에서 이미 차감
  (−div_sum원)" / ② **만기 후 확정 예정** — warning 색 + "롤 이후 월물 구간 — 캐리 숫자에
  미반영(그 월물 가격에 프라이싱)" / ③ **지난 1년 이력** — 기본 접힘 토글, "미공시 정기배당 힌트
  (확정 아님)". 셋 다 없으면 "최근 1년·향후 확정 배당 없음" 한 줄. 분류 헬퍼는
  `splitDivsByExpiry` / `divsAfterExpiry` 1벌 (목록 마커도 같은 함수).
