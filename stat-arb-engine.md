...
- **헤드라인 KPI 교체**: `half-life` → **"청산권(±0.3σ) 예상 N일(달력)"**. `예상거래일 = half-life거래일 × log₂(|현재z|/0.3)`, 달력일 = ×`CAL_PER_TRADING_DAY`(1.49, 연 ~245영업일/365). |z|≤0.3이면 "청산권 도달".
...
- ⚠️ 지수회귀라 **0σ 도달 불가**(∞) → 종착점은 청산권(0.3σ). half-life는 평균치라 큰 2σ 충격은 더 걸림(근사). 포지션 상세의 "예상 청산"은 기존 로직(추후 달력일 통일 여지).
