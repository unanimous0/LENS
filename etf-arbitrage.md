...
table width = 100% + minWidth 1846px. tableLayout fixed + col px 비례. 컨테이너 폭에 맞춰 모든 컬럼 비례 확장.

| # | 컬럼 | px | 공식 / 출처 |
|---|---|---:|---|
| 1 | 종목 (이름/코드, sticky-left) | 200 | ETF 마스터 |
| 2 | 거래대금 | 130 | `cum_volume` (1,234,567,890원 14자) |
| 3 | 현재가 | 86 | `etfTicks[code].price ?? prev_close` |
| 4 | **iNAV** | 100 | **거래소 발행 실시간 NAV (I5_ feed = `m.nav`). 모든 ETF 공통.** |
| 5 | rNAV | 100 | `(Σ(S·D) + cash) / CU` (자체 산출). **비차익 ETF는 PDF 처리 깨져 흐림** |
| 6 | fNAV | 100 | `(Σ(Z·D) + cash) / CU`, Z = V=O면 F, 아니면 S |
| 7 | 현재 괴리 | 80 | `(price − iNAV) / iNAV · 10000` bp |
| 8 | 매도 괴리 | 80 | `(ask1 − iNAV) / iNAV · 10000` bp |
| 9 | 매수 괴리 | 80 | `(bid1 − iNAV) / iNAV · 10000` bp |
| 10 | **매수차BP** | 88 | `Σ_{R>0, V=O} (T − slip − tax) · H` ≥ 0 |
| 11 | **매도차BP** | 88 | `Σ_{R<0, V=O} (T + slip) · H` ≤ 0 |
| 12 | 차익bp | 76 | `매수차BP + 매도차BP` (혼합에서 부호로 우세 방향) |
| 13 | **실집행차익(원)** | 130 | per-share, ETF 호가 vs fNAV (매수차는 매도세 차감) |
| 14 | **실집행BP** | 88 | `realProfitWon / rNAV · 10000` |
| 15 | **배당수** | 64 | 만기 이전 배당락 종목 수 |
| 16 | 선물비중 | 72 | V=O 종목 G 합 / rNAV_total · 100 |
| 17 | 선물수 | 64 | V=O 종목 수 |
| 18 | 추이 | 220 | sparkline (차익bp 5초 간격 history). 폭 = 150 sparkline + 좌우 여유 |

**iNAV vs rNAV 의미 분리** (LP 관점에서 진실값 비교):
- iNAV = 거래소 라이브 (모든 ETF 공통, 단일 진실원)
- rNAV = 자체 산출 (현물 1× 평가) — 차익 ETF는 두 값 비교로 차익 시그널, 비차익 ETF는 자체 산출이 깨져 흐림

비차익 ETF에서 rNAV 흐림 처리는 자체 산출이 의미 없는 경우 (지수선물 미평가 / 채권 시세 없음 / 옵션 프리미엄 영향 등). iNAV는 그대로 표시되어 LP 호가 작성에 활용 가능.
...
총 636 ETF 중 해외 73개 제외(`_OVERSEAS_KEYWORDS`) → **국내 563** (PR-13). 그중 차익 ~430 / 비차익 ~130 (레버리지/인버스/커버드콜/채권/혼합/통화/원자재/부동산/리츠 등). 커버드콜은 옵션 매도 프리미엄을 더해 NAV가 단순 현물 합산과 달라 PDF 기반 차익 산출 부정확.

**Frontend UI** — `etf-arbitrage.tsx`:
- `EtfMaster.arbitrable` 누락 시 true 폴백 (구버전 백엔드 호환)
- ArbRow `dim = nonArb || tooMissing` 통합:
  - **차익 컬럼만 흐림 + `—`**: rNAV/fNAV/매수차BP/매도차BP/차익bp/실집행차익/실집행BP/배당수/적용%/선물수/추이
  - **정상 표시**: 거래대금/현재가/iNAV/현재괴리/매도괴리/매수괴리
- **iNAV 컬럼**: 모든 ETF 공통 — 거래소 발행 실시간 NAV (I5_ feed = `m.nav`). rNAV 자체 산출과 비교용 진실값
- **rNAV 컬럼**: 항상 자체 산출 (`(SUM(S·D)+현금)/CU`). 비차익 ETF에선 PDF 처리가 깨져 (지수선물 미평가, 채권 시세 없음 등) 의미 없으므로 `—` 흐림. 차익 ETF에서 누락 임계 초과 시도 흐림.
- 정렬 시 차익 컬럼(`rNav`/`fNav`/`diffBp`/`buyArbBp`/`sellArbBp`/`realProfit*`/`dividendN`/`appliedPct`/`futuresCount`)에선 비차익 ETF 무조건 끝으로

**SubScopeToggles — sub 범위 제어** (PR-13, LS WS 동시 연결 한도 회피):
- 필터 패널 펼침 영역(`filterOpen=true`) 안에 토글 2줄 배치
- **표시 개수**: 50 / 100 (디폴트) / 200 / 300 / 전체 — visibleMaster.slice(0, limit). "전체" 선택 시 ⚠ 경고 배지 + `window.confirm()` 명시적 동의 (PR-15b) — stock+inav+orderbook 3 hook 동시 적용으로 conn ~70개 폭주 + LS 한도 초과 거의 확정이라 실수 클릭 방지
- **유형 필터**: 섹터형(디폴트) / 지수형 / 파생형 / 채권형 / 기타 + [전체] 버튼 — backend `_classify_etf(kr_name)` 기반 단일 분류
- localStorage 저장: `etf.subLimit`, `etf.subTypes`
- 30초마다 visibleMaster 자동 재계산 — 라이브 cum_volume으로 90~110위 변동 흡수 (ref-count + warm-down 60s가 sub/unsub 흡수 → LS reconnect 없음)
- **해외 ETF 제외**: backend `_OVERSEAS_KEYWORDS` (미국/나스닥/S&P/차이나/일본/유럽/(H) 등) 으로 ETF 마스터에서 제외 → ETF 페이지·LP 매트릭스 모두 국내형만. 636 → 563 ETF.
- **유형 분포** (`_classify_etf` 우선순위 단일 분류): derivative(레버리지/인버스/커버드콜) > bond(채권/회사채/MMF) > sector(반도체/자동차/금융 등 50+) > index(코스피200/코스닥150 등 정확한 지수명만) > other
...
PDF 기준일 = Finance_Data DB `etf_portfolio_daily.snapshot_date` (인포맥스 `/api/etf/port` 매일 적재, 5일 슬라이딩 윈도우). PDF는 주 1회 정도만 변경되지만 현금이 매일 변동해서 매일 fetch. 만기 잔존일과 배당 윈도우는 today 기준으로 항상 정확.
...
### 마스터 (정적, mount 시 1회)
- `/api/etfs` — ETF 마스터 + cu_unit (Finance_Data DB `etf_master_daily` 최신 snapshot)
- `/api/etfs/pdf-all` — PDF 구성종목 + 현금 (Finance_Data DB `etf_portfolio_daily` 최신 snapshot)
