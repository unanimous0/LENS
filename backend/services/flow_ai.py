...
_MAX_TOKENS = 2800  # 한국어 토큰 밀도 + 깊은 분석(요인별 해석 + 관찰 포인트)
...
_SYSTEM_PROMPT = (
    ...
    "(2) **사실 나열 금지 — 지표를 엮어 '왜 그런지'까지 해석하라.** 각 요인마다 그게 무슨 의미인지 한두 문장으로 풀어라. "
    "다음 해석은 해당되면 반드시 포함하라:\n"
    "  · **외국인 평단 대비 현재가(이익/손실 구간)와 함의** — 이익 구간이면 손절 압력이 낮아 하방이 견고, 손실 구간이면 물려 있어 추가 매집이 방어적일 수 있음.\n"
    "  · **외국인과 기관 중 누가 매집을 더 주도하는지** 비교 (매집률 수치로).\n"
    "  · **backtest_assessment(검증된 패턴별 초과수익)를 판단의 핵심 근거로.**\n"
    "  · **진입권 미충족이면** '외인·기관이 동반 매수하나 지속성 임계엔 못 미쳐 강하게 밀어붙이는 매수는 아님'으로 해석.\n"
    "  · **골든/데드크로스는 '누적 순매수(매집) 모멘텀의 전환'**으로 해석 (데드크로스=매집 모멘텀 둔화).\n"
    ...
    "(6) 한국어. 나열이 아니라 **분석**이다. 짧게 자르지 말고 각 요인을 충분히 설명하라. 구조:\n"
    "  ① 핵심 판단(한 줄, 이 종목이 지금 어떤 자리인지 규정) →\n"
    "  ② 강세 요인(핵심 2~3개, 각 요인에 왜 중요한지 해석) →\n"
    "  ③ 약세 요인(핵심 2~3개, 해석 포함) →\n"
    "  ④ **관찰 포인트**(무엇이 확인되면 강세가 굳어지고 무엇이 나오면 약세로 기우는지 한 줄) →\n"
    "  ⑤ '주의:'로 시작하는 맥락 한계 한 줄."
)
