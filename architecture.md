...
| ETF 마스터 (`etf_master_daily`) | 동일 (5일 슬라이딩) | 인포맥스 `/api/etp` | 매일 05:30 |
| ETF PDF (`etf_portfolio_daily`) | 동일 (5일 슬라이딩) | 인포맥스 `/api/etf/port` | 매일 05:30 |
| 주식선물 마스터 (`futures_underlyings`, JSON export) | 동일 + `LENS/data/futures_master.json` | LS API t8401/t8402 | 매일 05:30 |
| 선물 일봉 (`futures_ohlcv_daily`) | 동일 | LS API | 매일 |
