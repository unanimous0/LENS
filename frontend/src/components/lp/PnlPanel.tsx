    ...
    <div className="bg-bg-primary">
      <div className="px-3 py-2 border-b border-bg-base flex items-center justify-between">
        <div>
          ...
          <div className="text-[11px] text-t4">당일 세션(전일 종가 대비) P&amp;L 5분해 + markout + 한도 4개</div>
        </div>
        {pnl && (
        ...
        <>
          ...
          <div className="px-3 py-2.5 border-b border-bg-base">
            <div className="flex items-baseline justify-between mb-2">
              <span className="text-[10px] text-t3 uppercase tracking-wide">당일 P&amp;L 분해</span>
            ...
            {/* 지수 베이시스 산출 불가 상태 (정직) */}
            <div className="text-[10px] text-t4 mt-2">지수 베이시스: {pnl.basis_index_status}</div>
          </div>
          ...
          <div className="px-3 py-2.5 border-b border-bg-base grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="text-[9px] text-t4 mt-1">음수 = 역선택 (준 유동성 방향으로 불리)</div>
            </div>
            <div>
              <div className="text-[10px] text-t3 uppercase tracking-wide mb-1">스프레드 미귀속</div>
              ...
              <div className="text-[9px] text-t4 mt-1">fv_at_fill 없음 (비ETF·수동 기장) → 잔차 흡수</div>
            </div>
          </div>
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between">
        <span className="text-[10px] text-t3 truncate">{g.name}</span>
      ...
      <div className="flex items-baseline justify-between mt-0.5">
        <span className="text-[9px] text-t4 font-mono tabular-nums">
          {fmtSize(g.current)} / {g.limit > 0 ? fmtSize(g.limit) : '∞'}
        </span>
        {g.detail && <span className="text-[9px] text-t4 truncate ml-1">{g.detail}</span>}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="w-24 shrink-0">
        ...
        <div className="text-[9px] text-t4 leading-tight">{hint}</div>
      </div>
...
function Markout({ label, n, bp }: { label: string; n: number; bp: number }) {
  ...
  return (
    <div className="bg-bg-base/40 px-2 py-1.5 rounded-sm">
      <div className="text-[9px] text-t4">{label} ({n}건)</div>
